...
def query2str(items, max_length=1024):
    ...
    return '\n'.join([
        ...
        if k != axes_settings.PASSWORD_FORM_FIELD
...
def get_ip(request):
    ...
    if axes_settings.BEHIND_REVERSE_PROXY:
        ...
        ip_str = request.META.get(axes_settings.REVERSE_PROXY_HEADER, '')
        ...
        if len(ip_list) >= axes_settings.NUM_PROXIES:
            ip = ip_list[-axes_settings.NUM_PROXIES]
        ...
        # If nth last is not found, default to no IP and raise a warning
            ...
            raise Warning(
                ...
                'Header name: {0}, value: {1}'.format(
                    axes_settings.REVERSE_PROXY_HEADER, ip_str
                )
            )
